...
Logo $\lambda=2$ e $\lambda=3$são as raizes do polinomio caracteristico de $\mathnormal{A}$, e portanto os autovalores da matriz $\mathnormal{A}$ são 2 e 3. Conhecendo os autovalores podemos encontrar os autovetores correspondentes. Resolvendo a equação: $\mathnormal{A}\mathnormal{v}=\lambda\mathnormal{v}$\vspace*{3mm}\newline
$\lambda=2$\newline
...
\end{bmatrix} \implies  \begin{cases}
  ...
  Implicando em: $y=0$, x=0 e como nenhuma equação impõe restrição em z, os autovetores associados a $\lambda=2$ são do tipo $\mathnormal{v}(0,0,z)$ pertendo ao subespaço[(0,0,1)]. \vspace*{3mm}\newline
 $\lambda=3$\newline
 $\begin{bmatrix*} 4& -2 &  0\\  -1& 1 & 0 \\  0& 1 & 2 \end{bmatrix*} \begin{bmatrix}
x\\y\\z\\
...
\end{bmatrix} \implies  \begin{cases}
  ...
  \end{cases}$\vspace*{3mm}\newline
  Implicando em:y=z,x=2y. Os autovetores associados a $\lambda=3$ são do tipo (-2y, y, y) pertencendo ao  subespaço[(-2,1,1)].\newline
  Generalizando:\vspace*{3mm}\newline
  $ \begin{bmatrix}
